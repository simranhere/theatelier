...
export default function Signup() {
  ...
  function doSignup() {
    ...
    axios.post("https://theatelier-wheat.vercel.app/user/signup", form)
      ...
      .catch(() => setServerMessage("Error signing up"));
  }
  ...
  return (
    ...
    >
      ...
      <div className="min-h-screen bg-black/10 flex flex-col lg:flex-row items-center justify-center px-4 py-10 gap-8 lg:gap-0">

        {/* LEFT SECTION — hidden on very small screens, shown from md up */}
        <div className="hidden md:flex flex-col items-center lg:items-start justify-center lg:w-1/2 lg:pl-[10%] xl:pl-[15%]">
          <h1
            className="text-4xl xl:text-6xl text-[#2A2A2A] text-center lg:text-left"
            style={{ fontFamily: "Playfair Display, serif" }}
          >
            Join The Atelier
          </h1>
          <p className="mt-4 text-base lg:text-lg text-[#6B5B3E] text-center lg:text-left">
            Tailored just for you. <br /> Begin your bespoke journey today.
          </p>
          <motion.img
            key={form.utype}
            ...
            animate={{ opacity: 1 }}
            transition={{ duration: 0.4 }}
            className="mt-8 w-40 h-40 lg:w-56 lg:h-56 xl:w-64 xl:h-64 object-cover rounded-full border-4 border-[#C6A75E] shadow-lg"
          />
        </div>
        ...
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6 }}
          className="w-full max-w-sm sm:max-w-md lg:w-[420px] lg:mr-[8%] bg-white/50 backdrop-blur-md p-6 sm:p-10 rounded-xl shadow-xl"
        >
          {/* Show title on mobile since left section is hidden */}
          <div className="md:hidden text-center mb-4">
            ...
            <p className="mt-2 text-sm text-[#6B5B3E]">
              Tailored just for you. Begin your bespoke journey today.
            </p>
          </div>

          <h2
            className="text-2xl sm:text-3xl text-center mb-6 sm:mb-8 text-[#2A2A2A]"
            style={{ fontFamily: "Playfair Display, serif" }}
          >
            Signup
          </h2>
          ...
          <div className="mb-5">
            ...
            <input
              ...
              onChange={handleChange}
              onBlur={() => setTouched({ ...touched, emailid: true })}
              className={`w-full px-4 py-3 rounded-lg bg-[#F8F3E8] border text-sm ${
          ...
          <div className="mb-5 relative">
            ...
            <div className="relative">
              <input
                ...
                onChange={handleChange}
                onBlur={() => setTouched({ ...touched, pwd: true })}
                className="w-full px-4 py-3 pr-10 rounded-lg bg-[#F8F3E8] border border-[#D6C7A8] text-sm"
              />
              <button
                type="button"
                onClick={() => setShowPassword(!showPassword)}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-[#7A5C2E]"
              >
                {showPassword ? <EyeOff size={18} /> : <Eye size={18} />}
              </button>
            </div>
          ...
          <div className="mb-6">
            ...
            {touched.utype && !userTypeValid && (
              <p className="text-red-500 text-xs mt-1">Please select a user type.</p>
            )}
          </div>
          ...
          <button
            ...
            onClick={doSignup}
            disabled={!emailValid || !passwordValid || !userTypeValid}
            className="w-full py-3 rounded-full text-white font-medium bg-gradient-to-r from-[#C6A75E] to-[#A8843C] hover:scale-105 transition-all disabled:opacity-40 disabled:cursor-not-allowed text-sm"
          >
            Signup
          </button>
          ...
          {serverMessage && (
            <p
              className={`text-sm text-center mt-4 ${
                serverMessage.includes("successful") ? "text-green-700" : "text-red-500"
              }`}
            >
              {serverMessage}
            </p>
          ...
          <p
            className="text-center mt-5 text-sm text-[#6B5B3E]"
